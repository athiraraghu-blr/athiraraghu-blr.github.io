...
1. Embrace Async All the Way Down
...
async def get_user(user_id: int):
    user = db.execute("SELECT * FROM users WHERE id = ?", user_id)  # sync!
    return user
...
2. Use Multiple Workers and the Right Server
...
4. Caching: The Single Biggest Throughput Multiplier
...
6. Background Tasks for Non-Critical Work
...
8. Efficient Dependency Injection
...
Make all I/O non-blocking (async libraries, no sync calls in async handlers)

Add connection pooling for databases and external services

Cache aggressively at the right layer (in-memory → Redis → CDN)

Use multiple workers to exploit all CPU cores

Switch to ORJSONResponse and Pydantic v2 for faster serialization

Move non-critical work to background tasks or a task queue

Add rate limiting and throttling at the gateway

Profile and optimize database queries — indexes, eager loading, pagination

Instrument with metrics and tracing to validate improvements
